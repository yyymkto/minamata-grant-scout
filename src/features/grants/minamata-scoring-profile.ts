...
export const UNIQUENESS_TAGS: Record<UniquenessTagKey, UniquenessTagDef> = {
  MINAMATA_DISEASE_AREA: {
    ...
    bonus: 15,
    appliesWhen:
      "国・県の交付要綱等で、対象地域・対象者が「水俣病発生地域」「胎児性・小児性水俣病患者」等に明示的に限定されている場合のみ。単に熊本県内の自治体・施設が対象になりうるだけの一般的な補助金（震災復旧、老朽化対策、処遇改善、施設整備等）には絶対に付けないこと",
    examples: [
    ...
    doNotApplyTo: [
      "熊本県内の介護・福祉施設向け一般的な震災復旧費補助金（対象地域が熊本県全域等で水俣病発生地域に限定されていないもの）",
      "全国・熊本県内の事業者向け一般的なエネルギー・環境系補助金（水俣病や水俣病発生地域への言及が本文に無いもの）",
      "処遇改善・人材確保等、熊本県内で広く対象になる補助金",
    ],
    confidence: "high",
  },
  MOYAI: {
